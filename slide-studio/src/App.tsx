...
function PreviewDialog({ projectId, job, onClose, onAccept, onReject }: { projectId: string; job: AiJob | null; onClose: () => void; onAccept: () => Promise<void>; onReject: () => Promise<void> }) {
  const [busy, setBusy] = useState(false);
  return <div className="modal-backdrop"><section className="preview-dialog"><header><div><p className="eyebrow">REVIEW BEFORE APPLY</p><h2>{job ? "Codex 변경안 비교" : "현재 Draft 미리보기"}</h2></div><button onClick={onClose}>×</button></header><div className={`preview-grid ${job ? "" : "single"}`}><div><span>현재 Draft</span><iframe src={`/api/projects/${projectId}/preview?at=${Date.now()}`} title="현재 Draft" /></div>{job && <div><span>Codex 변경안</span><iframe src={`/api/ai-jobs/${job.id}/preview?at=${Date.now()}`} title="Codex 변경안" /></div>}</div>{job && <footer><pre>{job.summary}</pre><div><button disabled={busy} onClick={async () => { setBusy(true); await onReject(); }}>폐기</button><button className="primary" disabled={busy} onClick={async () => { setBusy(true); await onAccept(); }}>Draft에 적용</button></div></footer>}</section></div>;
}
